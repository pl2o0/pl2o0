...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        if Config.Stores[k].shopType == "clothing" then
            ...
            AddTextComponentString("<FONT FACE = 'Arb'>".."ﺲﺑﻼﻤﻟﺍ ﻞﺤﻣ")
            EndTextCommandSetBlipName(clothingShop)
        end
        if Config.Stores[k].shopType == "changer" then
            local clothingShop = AddBlipForCoord(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z)
            SetBlipSprite(clothingShop, 366)
            SetBlipColour(clothingShop, 20)
            SetBlipScale  (clothingShop, 0.7)
            SetBlipAsShortRange(clothingShop, true)
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("<FONT FACE = 'Arb'>".."ﺲﺑﻼﻤﻟﺍ ﻞﺤﻣ")
        ...
        if Config.Stores[k].shopType == "barber" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("<FONT FACE = 'Arb'>".." ﺔﻗﻼﺣ ﻥﻮﻟﺎﺻ")
            ...
            for k, v in pairs(Config.Stores) do
                ...
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - ﺲﺑﻼﻤﻟﺍ ﺮﻴﻴﻐﺗ')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].x, Config.Stores[k].y, Config.Stores[k].z + 1.25, '~g~E~w~ - ﺮﻌﺸﻟﺍ ﺮﻴﻴﻐﺗ')
                            end
                            if IsControlJustPressed(0, Keys["E"]) then
                                if Config.Stores[k].shopType == "clothing" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "character", label = "الملابس", selected = true},
                                        {menu = "accessoires", label = "الاكسسوارات", selected = false}
                                    })
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "الشعر", selected = true},
                                    })
                                end
                            end
                        end
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].x, Config.ClothingRooms[k].y, Config.ClothingRooms[k].z + 0.3, '~g~E~w~ - ﺲﺑﻼﻤﻟﺍ ﻢﻘﻃﺍ ﺪﻫﺎﺷ')
                                if IsControlJustPressed(0, Keys["E"]) then
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "اعدادات مسبقة", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender]},
                                            {menu = "myOutfits", label = "ملابسي", selected = false, outfits = result},
                                            {menu = "character", label = "الشخصية", selected = false},
                                            {menu = "accessoires", label = "الاضافات", selected = false}
                                        })
...
AddEventHandler('qb-clothing:client:openOutfitMenu', function()
    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
        openMenu({
            {menu = "myOutfits", label = "ملابسي", selected = true, outfits = result},
        })
    end)
end)
...
AddEventHandler('qb-clothing:client:openMenu', function()
    customCamLocation = nil
    openMenu({
        {menu = "spawn", label = "تصميم الشخصية", selected = true},
        {menu = "character", label = "الشخصية", selected = false},
        {menu = "clothing", label = "المظهر", selected = false},
        {menu = "accessoires", label = "الاكسسوارات", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    TriggerEvent('chatMessage', "SYSTEM", "error", ".. ﻞﺣﺎﻜﻟﺍ ﻁﺎﺑﺭ ﻊﻠﺧ ﻚﻨﻜﻤﻳ ﻻ")
end)
...
RegisterNUICallback('removeOutfit', function(data, cb)
    ...
    TriggerEvent('chatMessage', "SYSTEM", "warning", "You have deleted "..data.outfitName.."!")
end)
